...
=======
  void SetParameter(const unsigned int iparam, const pidvar_t value)
  {
    mPar[iparam] = value;
  }
...
=======
  const pidvar_t* GetParameters() const
  {
    return mPar.to_array();
  }
...
<<<<<<< HEAD
   ...
   private:
    ...
    std::string mName;
=======
  pidvar_t operator[](const unsigned int i) const
  {
    return mPar[i];
  }
